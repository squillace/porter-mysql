...
func TestSchema(t *testing.T) {
	schemaBackup := "../pkg/mysql/schema/schema.json.bak"
	schemaPath := "../pkg/mysql/schema/schema.json"
	...
	output := &bytes.Buffer{}
	cmd := exec.Command("../bin/mixins/mysql/mysql", "schema")
	cmd.Stdout = output
	cmd.Stderr = output

	err = cmd.Start()
	require.NoError(t, err, "failed to start the mysql schema command")

	err = cmd.Wait()
	t.Log(output)
	require.NoError(t, err, "mysql schema failed")
}
